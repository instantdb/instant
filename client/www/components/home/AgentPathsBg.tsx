...
function createAgent(w: number, h: number): Agent {
...
export function AgentPathsBg() {
  ...
  useEffect(() => {
    ...
    function init() {
      agents = Array.from({ length: NUM_AGENTS }, () => createAgent(cw, ch));
    ...
    function tick() {
      ...
      atmGrad.addColorStop(0, 'rgba(255, 237, 213, 0.35)');
      atmGrad.addColorStop(0.5, 'rgba(254, 215, 170, 0.12)');
      ...
      for (const agent of agents) {
        ...
        ctx!.fillStyle = `rgba(${cr}, ${cg2}, ${cb}, 0.75)`;
        ctx!.fill();
        ...
        hg.addColorStop(0, `rgba(${cr}, ${cg2}, ${cb}, 0.3)`);
