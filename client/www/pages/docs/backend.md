...
If using NextJS you can call getUnvalidatedUserFromInstantCookie with the app id to retrieve the user in any server component, or route handler.
...
export default async function RootLayout({
  ...
  const user = await getUnvalidatedUserFromInstantCookie(process.env.NEXT_PUBLIC_INSTANT_APP_ID!);
