...
async function createApp(title, orgId?: string) {
  const id = randomUUID();
...
async function detectAppWithErrorLogging(opts): Promise<any> {
  const fromOpts = await detectAppIdFromOptsWithErrorLogging(opts);
  if (!fromOpts.ok) return fromOpts;
  if (fromOpts.appId) {
    return { ok: true, appId: fromOpts.appId as string, source: 'opts' };
  }
...
async function readLocalSchemaFile() {
  ...
  if (!res.config) return;
  const relativePath = path.relative(process.cwd(), res.sources[0]);
  return { path: relativePath, schema: res.config as any };
}
...
async function detectAppIdFromOptsWithErrorLogging(opts) {
  ...
  const config = (await readInstantConfigFile()) as any;
