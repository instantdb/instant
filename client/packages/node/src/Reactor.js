...
// Import from @instantdb/core - we'll need to export these from core
import {
  instaql,
  instaml,
  store as s,
...
import NodeWebSocket from './adapters/NodeWebSocket.js';

/** @typedef {import('@instantdb/core/src/utils/log.ts').Logger} Logger */
...
function createWebSocket(uri) {
  const ws = new NodeWebSocket(uri);
  // @ts-ignore
  ws._id = _wsId++;
  return ws;
}

// Modified for Node.js - always return false
function isClient() {
  return false;
}
...
/**
 * @template {import('@instantdb/core/src/presence.ts').RoomSchemaShape} [RoomSchema = {}]
 */
export default class Reactor {
  ...
  /** @type {null | import('@instantdb/core/src/utils/linkIndex.ts').LinkIndex}} */
  _linkIndex = null;
  ...
  constructor(
    ...
    NetworkListener = createNodeNetworkListener,
    versions,
  ) {
    ...
    // No BroadcastChannel in Node.js
    this._broadcastChannel = undefined;

    // No OAuth callback in Node.js
    this._oauthCallbackResponse = Promise.resolve(null);
    ...
    // Use Node.js network listener
    const networkListener = NetworkListener();
    this._isOnline = networkListener.isOnline();
    this._startSocket();
    ...
    // No beforeunload event in Node.js
  }
  ...
  _initStorage(Storage) {
    this._persister = Storage(`instant_${this.config.appId}_5`);
  ...
  async changeCurrentUser(newUser) {
    ...
    // No broadcast channel in Node.js
  }
