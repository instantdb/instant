...
CREATE TABLE instant_user_refresh_tokens(
  ...
  CONSTRAINT fk_instant_user_id FOREIGN KEY(user_id) REFERENCES instant_users(id) ON DELETE CASCADE
);
...
CREATE TABLE instant_user_magic_codes(
  ...
  user_id uuid NOT NULL,
  created_at TIMESTAMP NOT NULL DEFAULT NOW(),
  CONSTRAINT fk_user_id FOREIGN KEY(user_id) REFERENCES instant_users(id) ON DELETE CASCADE
);
...
CREATE TABLE app_user_refresh_tokens(
  ...
  user_id uuid NOT NULL,
  created_at TIMESTAMP NOT NULL DEFAULT NOW(),
  CONSTRAINT fk_app_user_id FOREIGN KEY(user_id) REFERENCES app_users(id) ON DELETE CASCADE
);
...
CREATE UNIQUE INDEX ea_index ON triples(app_id, entity_id, attr_id)
WHERE
  ea;

CREATE UNIQUE INDEX eav_index ON triples(app_id, entity_id, attr_id, value)
WHERE
  eav;

CREATE UNIQUE INDEX av_index ON triples(app_id, attr_id, value) INCLUDE (entity_id)
WHERE
  av;

CREATE INDEX ave_index ON triples(app_id, attr_id, value, entity_id)
WHERE
  ave;

CREATE INDEX vae_index ON triples(app_id, value, attr_id, entity_id)
WHERE
  vae;
...
ADD
  CONSTRAINT ref_values_are_uuid CHECK (
    CASE
      WHEN eav
      OR vae THEN (value ->> 0) :: uuid IS NOT NULL
      ELSE TRUE
    END
  );
...
ALTER TABLE
  triples
ADD
  CONSTRAINT indexed_values_are_constrained CHECK (
    CASE
      WHEN eav
      OR av
      OR ave
      OR vae THEN pg_column_size(value) <= 1024
      ELSE TRUE
    END
  );
