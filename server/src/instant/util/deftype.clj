(ns instant.util.deftype)

(def type-registry (atom {}))

(defmacro deftype-once [name fields & body]
  (let [fingerprint (hash [fields body])
        registry-key [(ns-name *ns*) name]
        constructor (symbol (str "->" name))
        already-defined? (ns-resolve *ns* constructor)
        matches-last-known? (= (get @type-registry registry-key) fingerprint)]
    (if (and already-defined? matches-last-known?)
      nil
      (do
        (swap! type-registry assoc registry-key fingerprint)
        `(deftype ~name ~fields ~@body)))))
