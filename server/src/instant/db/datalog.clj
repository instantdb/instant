...
(comment
  (join-cols nil :match- 1 [:v :v]))

(defn- join-cond
  "Generates a single join condition,
   given the origin component type and the destination path"
  [table prefix origin-ctype [dest-idx dest-col-idx]]
  (let [dest-ctype (idx->component-type dest-col-idx)
        [origin-col dest-col] (join-cols table prefix dest-idx [origin-ctype dest-ctype])]
    [:= origin-col dest-col]))
...
(defn- join-cond-for-or
  ...
  [table prefix ctype paths]
  (list* :or
         (map (fn [paths]
                (if (set? paths)
                  (join-cond-for-or table prefix ctype paths)
                  (list* :and
                         (map (fn [path]
                                (if (set? path)
                                  (join-cond-for-or table prefix ctype path)
                                  (join-cond table prefix ctype path)))
                              paths))))
              paths)))
...
(defn join-conds
  ...
  [table prefix symbol-map named-p]
  (->> named-p
       variable-components
       (keep (fn [[ctype [_ sym]]]
...
(defn- joining-with
  ...
  (let [cur-idx (inc prev-idx)
        ...
        joins (if start-of-group?
                []
                (join-conds nil prefix symbol-map named-p))
        parent-joins (->> named-p
                          variable-components
                          (mapcat (fn [[ctype [_ sym]]]
