...
(defn transact! [span-name conn tx-data]
  (let [report (if (or (flags/toggled? :enable-store-batching-globally)
    ...
    (if-let [eids (seq (deleted-subscriptions-datalog-query-ids report))]
      (transact! "clean-datalog-queries" conn [[:db.fn/call clean-stale-datalog-queries-tx-data (vec eids)]])
      report)))
...
(defn- swap-datalog-cache-tx-data
  ...
  (let [existing (d/entity db [:datalog-query/app-id+query [app-id query]])
        _ (when-not existing (throw (Exception. "Nobody cares about this datalog query")))
        watchers (:datalog-query/watchers existing)
        id (:db/id existing)]
    (if (or (not watchers)
