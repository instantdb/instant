...
(defn- handle-refresh! [store-conn sess-id app-id event debug-info]
...
(defn- handle-join-room! [store-conn app-id sess-id {:keys [client-event-id room-id] :as _event}]
  (let [auth (get-auth! store-conn app-id sess-id)
        app-id (-> auth :app :id)
        current-user (-> auth :user)]
...
(defn- handle-leave-room! [store-conn app-id sess-id {:keys [client-event-id room-id] :as _event}]
  (let [auth (get-auth! store-conn app-id sess-id)
        app-id (-> auth :app :id)]
...
(defn- handle-set-presence!
  [store-conn app-id sess-id {:keys [client-event-id room-id data] :as _event}]
...
(defn- handle-refresh-presence! [store-conn app-id sess-id {:keys [app-id room-id data edits]}]
...
(defn- handle-client-broadcast!
  ...
  [store-conn app-id sess-id {:keys [client-event-id room-id topic data] :as _event}]
  (let [auth (get-auth! store-conn sess-id)
        _ (assert-in-room! app-id room-id sess-id)
        current-user (-> auth :user)
...
(defn- handle-server-broadcast! [store-conn app-id sess-id {:keys [room-id topic data]}]
...
(defn handle-event [store-conn app-id session event debug-info]
  (let [{:keys [op]} event
    ...
    (case op
      ...
      :refresh (handle-refresh! store-conn app-id id event debug-info)
      :transact (handle-transact! store-conn app-id id event)
      :error (handle-error! store-conn app-id id event)
      ;; -----
      ;; EPH events
      :join-room (handle-join-room! store-conn app-id id event)
      :leave-room (handle-leave-room! store-conn app-id id event)
      :set-presence (handle-set-presence! store-conn app-id id event)
      :refresh-presence (handle-refresh-presence! store-conn app-id id event)
      :client-broadcast (handle-client-broadcast! store-conn app-id id event)
      :server-broadcast (handle-server-broadcast! store-conn app-id id event))))

;; --------------
;; Receive Workers

(defn- handle-instant-exception [store-conn app-id session original-event instant-ex debug-info]
...
(defn handle-receive [store-conn app-id session event metadata]
...
(defn process-receive-q-entry [store-conn entry metadata]
  (let [{:keys [put-at item skipped-size]} entry
        {:keys [session-id app-id] :as event} item
        now (Instant/now)
...
(defn on-close [store-conn {:keys [app-id id pending-handlers]}]
  ...
  (let [app-id (or app-id (-> (rs/get-auth @store-conn id)
                              :app
                              :id))]
