      ...
      (if-not (clj-http/success? resp)
        ...
        (let [id-token (try
          ...
          (if-not token-data
            ...
            (let [;; Use provider registry to extract user info
                  user-info (providers/extract-user-info provider-name token-data)
                  {:keys [sub email raw-data]} user-info]
              (tracer/with-span! {:name "oauth/user-info-extracted"
                                  :attributes {:provider provider-name
                                               :has-email (boolean email)
                                               :has-sub (boolean sub)
                                               :extracted-sub sub
                                               :extracted-email email
                                               :raw-response (pr-str raw-data)}}
                (if (and email sub)
                  {:type :success :email email :sub sub}
                  {:type :error
                   :message (str "Missing user info - email: " (boolean email) ", sub: " (boolean sub))}))))))))
